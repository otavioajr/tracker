...
- **Gateway** (`gateway/`) — Go TCP server that receives GPS device data (Suntech protocol), buffers positions, evaluates alert rules, writes to PostgreSQL
- **Web** (`web/`) — Next.js 16 dashboard with real-time map tracking, device/vehicle CRUD, history playback, geofences, alerts, reports
- **Simulator** (`simulator/`) — Go tool that generates fake GPS positions and sends them to the gateway via TCP
...
- `src/lib/db/` — Drizzle ORM client (schema at `src/lib/db/schema.ts`)
- `src/lib/hooks/` — Real-time position subscription hooks
- `src/components/map/` — Leaflet map components (dynamically imported for SSR)
- `src/components/ui/` — Shadcn UI components (Base UI variant)
- `src/types/database.ts` — Auto-generated Supabase types (via `make db-types`)

UI stack: Tailwind CSS v4, Shadcn UI (base-nova style), next-themes for dark mode, Leaflet/react-leaflet for maps.

### Database

6 migrations in order: extensions/enums → tenants/profiles → devices/vehicles → positions (time-partitioned) → geofences/alerts → RLS policies.
