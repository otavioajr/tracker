...
func NewCommandHTTPServer(addr, token string, queue *Server) (*http.Server, error) {
	...
	mux.HandleFunc("/cmd", func(w http.ResponseWriter, r *http.Request) {
		...
		// Even localhost requests must not be triggered by a browser on a hostile site.
		...
		// GET is retained only for existing local scripts. New callers should use POST.
		...
		if imei == "" || command == "" {
			// Preserve the legacy error text for existing console scripts.
		...
		// Match the installed binary's response; this acknowledges queuing only.
