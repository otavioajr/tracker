...
func TestCommandDeliveredAfterGT06LoginACK(t *testing.T) {
	...
	// A second login must not cause already drained commands to be sent again.
	conn.Write(login)
...
func TestCommandFailureDoesNotDropPosition(t *testing.T) {
	...
	// Enqueue from the position callback, avoiding a race with login's queue flush.
	gps, _ := hex.DecodeString("787817121A03120A1E00800286D5740500D2642D0C7F0001AAAA0D0A")
	if _, err := client.Write(gps); err != nil {
		t.Fatal(err)
	}
	// A pipe write confirms the reading arrived; closing now fails the outgoing command.
	client.Close()
	select {
	case <-done:
...
func TestConcurrentCommandEnqueueIsBounded(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		wg.Add(1)
		go func() { defer wg.Done(); _ = srv.EnqueueCommand("358899050127810", "STATUS#") }()
	}
	wg.Wait()
